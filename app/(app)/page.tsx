...
function ChatPage() {
    ...
    const startRun = async (threadId: string, assistantId: string):Promise<string> => {
        try {
            const { data: {success, run} } = await axios.post<{
                success: boolean,
                error?: string,
                run?: Run
            }>('/api/run/create', {
    ...
    const pollRunStatus = async (threadId: string, runId: string) => {
        ...
        const intervalId = setInterval(async() => {
            try {
                const {data: {run, success}} = await axios.post<{success: boolean, error?: string, run?: Run}>("/api/run/retrieve", {
                    threadId,
                    runId
                })
    ...
    return (
        <div className="w-full flex flex-col bg-black text-white pb-20">
